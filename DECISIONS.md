...
## Gotchas
- BPs live in `~/repos/SICIP-BPs`; its pipeline reads this repo's `data/data.json` + `web/fonts/` by absolute path.
...
- Air-conditioning halving rule: sheets whose xlsx carries a `*`-prefixed footnote row under the points row get `note` in data.json, a footnote in Conditions and an "Is air-conditioned" checkbox in the score card; unchecking halves Points/30 only (Score/100 and the 80% eligibility are untouched). `gen_data.read_sheet` scans 12 rows past the equipment block for it; `build_sheet` writes it from `co['note']`. Footnotes of the form "at least N <items>. Otherwise … halved" (6 welding sheets: 8 welding booths) (also "at least N x and M y") become `min_rules [{row,min,item}]` via `gen_data.min_rules`; 5 construction EIM sheets: 15 channel + 3 concealed wiring booths — halving is automatic from that row's Available, no checkbox. xlsx Points formula is NOT halved for either rule (footnote only).
- Adding a course: PDF into `competency-standards/<ORG>/`, sheet into the org workbook via `build_sheet`, `<li>` + org-count in `docs/reference-lab-index.html`, entry in `data/peer-data.json` (approved + cs_pdf), bump the entry-count assert in `gen_data.py`, then `gen_data.py` + `split_sheets.py` (revert the untouched per-course xlsx).
...
2026-09-14 | POQA 1-All-Standards IT sheets ported into 8 lab standards (PKSF/BMET/BWCCI/WEAB Graphic Design, IT Freelancing, Digital Marketing): space row = classroom 350 + lab 500 OR combined 500; full PC/software config (FHD monitor, InDesign/After Effects/3D, SEO/CRM/affiliate/ad-campaign tools); AC halving footnote + calculator toggle. Our item lists and weights kept; the Graphic `Air-conditioning` w5 row dropped (halving replaces it, Σ52 -> 47) | user: POQA sheets vetted through practical experience
2026-09-14 | BSIA Semiconductor Testing Engineering added (163rd): CS `6. BSIA_Semiconductor Testing Engineering_31 Aug 26.pdf`, 20 items Σw 151, weights by analogy with the other 5 BSIA sheets (CS carries none); approved 31 Aug 2026 from the filename (cover page says 01 September 2026) | user supplied the CS
2026-09-14 | PKSF Housekeeping added (164th) as a copy of the ISC-T&H standard: same 27 items Σw 140, ISC-T&H CS PDF copied into `competency-standards/PKSF/` under its original name so the shared provenance stays visible | user: PKSF runs the same course
2026-09-14 | Semiconductor Testing: Computer/Laptop, Printer, Multimedia Projector weights 8/6/7 -> 5/5/5 (Σ161 -> 155) | user: repo convention is 5 for these (projector 5 in 105 of 111 rows, computer/laptop 5 in most); the other five BSIA sheets still carry the 8/7/6 outliers, and Internet connectivity is 5 in 34 of 40 rows but 6 here — both left alone pending the user's call
2026-09-14 | Semiconductor Testing: trainer computer + 25 workstations merged into one row, required 26 (25 trainees + 01 trainer), weight 10, listed first; 21 items Σ155 -> 20 items Σ151 | user
...
2026-09-23 | CPU spec → 'Minimum Intel core i5 or Intel core i3 6 gen/above, or AMD similar spec' in BASIS UI/UX, BWCCI/WEAB DM, PKSF DM + IT Freelancing (BACCO untouched) | user request, align with graphic-design spec
2026-09-23 | Graphic design (BMET/BWCCI/WEAB/PKSF) CPU: i3 6 gen → 8 gen/above | user request
2026-09-23 | AMD wording → 'or AMD Ryzen similar spec' in all 9 design/DM/freelancing/UI-UX labs (no 'Ryzen 5') | user request
