...
## Next
- BP similar-course analysis (Business-Plan/analysis/Similar-Course-Analysis-report.html + slides.html (gitignored, local only)): 3 orgs lack BPs (LFMEAB, BASIS, BSIA); BITAC covered by 2 scanned BPs (OCR), Short-course BP's 4 basic courses not in data.json; AEOSIB only via OCR (financial tables unreadable); Asia-TTC BP covers 3 courses. Get those BPs to fill blanks; regen = `python3 Business-Plan/analysis/pipeline/{groups,merge,gen,xlsx}.py` (xlsx = similar-courses.xlsx).
