...
- **Click a course → its lab-standard page**, which is template-faithful:
  ...
  - **Download .xlsx** → serves the **exact template workbook** file for that org (`lab-standards/<ORG>-lab-standard.xlsx`). Asks for confirmation.
...
data/                     data.json (generated) for the site
